			...
			main: {
				files: [
					...
					{expand: true, cwd: 'test/fixtures', src: ['**', '!*.wav'], dest: 'tmp/copy_test_mix/'},
					{
						...
						cwd: 'test/fixtures',
						src: ['<%= testVars.match %>'],
						dest: 'tmp/copy_test_v<%= testVars.version %>/'
					}
				]
			},

			/** force overwrite of read-only files */
			mainCreateFile: {
				options: {
					force: true,
					mode: true //copies read-only flag
				},
				files: [
					{expand: true, cwd: 'test', src: ['readonly/readonly.js'], dest: 'tmp/copy_test_readonly'}
				]
			},
			readOnlyFile: {
				options: {
